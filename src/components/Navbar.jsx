...
const SearchBar = () => {
	...
	const handleChange = (e) => {
		...
		debounceRef.current = setTimeout(async () => {
			try {
				const res = await api.get(`/auth/search/?q=${encodeURIComponent(val)}`)
				setResults(res.data)
				setOpen(true)
			} catch {}
				...
				<div className="search-dropdown">
					{loading ? (
						...
						results.map(u => {
							...
							return (
								<div key={u.id} className="search-item" onClick={() => handleSelect(u.id)}>
									...
									<div className="search-info">
										...
										{u.bio && <span>{u.bio.slice(0, 40)}{u.bio.length > 40 ? '…' : ''}</span>}
									</div>
								</div>
							)
						})
...
const Navbar = () => {
	...
	const handleNotifClick = async (n) => {
		...
		const postTypes = ['like_post', 'comment', 'reply', 'like_comment', 'like_reply']
		if (postTypes.includes(n.type) && n.post_id) {
			navigate(`/profile/${n.post_author_id}?post=${n.post_id}`)
		} else {
			...
			<div className="topbar-inner">
				<div className="topbar-left">
					<Link to="/" className="topbar-brand">
						<img src="/sweety-icon.svg" alt="Sweety" className="topbar-icon" />
				...
				<nav className="topbar-nav">
					<Link to="/feed" className={`nav-icon ${pathname === '/feed' ? 'active' : ''}`} title="Home">
						<svg width="20" height="20" viewBox="0 0 18 21" fill="none">
							<path stroke="currentColor" strokeWidth="1.5" d="M1 9.924c0-1.552 0-2.328.314-3.01.313-.682.902-1.187 2.08-2.196l1.143-.98C6.667 1.913 7.732 1 9 1c1.268 0 2.333.913 4.463 2.738l1.142.98c1.179 1.01 1.768 1.514 2.081 2.196.314.682.314 1.458.314 3.01v4.846c0 2.155 0 3.233-.67 3.902-.669.67-1.746.67-3.901.67H5.57c-2.155 0-3.232 0-3.902-.67C1 18.002 1 16.925 1 14.77V9.924z" />
							<path stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" d="M11.857 19.341v-5.857a1 1 0 00-1-1H7.143a1 1 0 00-1 1v5.857" />
						</svg>
					</Link>

					<Link to="/find-friends" className={`nav-icon ${pathname === '/find-friends' ? 'active' : ''}`} title="Friends">
					...
					<div className="nav-icon-wrap" ref={notifRef}>
						<button className="nav-icon nav-icon-badge" title="Notifications" onClick={() => setNotifOpen(!notifOpen)}>
										...
										filtered.map((n) => (
											...
											>
												<div className="notif-img-wrap">
													{n.sender.avatar ? (
														<img src={n.sender.avatar} alt="" className="notif-img" />
													) : (
														<div className="notif-avatar-initials">{n.sender.initials}</div>
													)}
													{!n.is_read && <span className="notif-dot" />}
												</div>
												<div className="notif-body">
													<p><strong>{n.sender.name}</strong> {NOTIF_TEXT[n.type]}</p>
					...
					{/* Message Icon */}
					<a href="#" className="nav-icon nav-icon-badge" title="Messages">
				...
				<div className="topbar-right">
					<div className="user-menu" ref={profileRef} onClick={() => setProfileOpen(!profileOpen)}>
						...
						{profileOpen && (
							<div className="user-dropdown">
								...
								<hr />
								<button className="dropdown-item" onClick={() => { setProfileOpen(false); navigate(`/profile/${user?.id}`) }}>
								...
								<button className="dropdown-item" onClick={() => { setProfileOpen(false); navigate('/help') }}>
									<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="2">
								...
								{user?.is_staff && (
									<button className="dropdown-item" onClick={() => { setProfileOpen(false); navigate('/admin') }}>
										<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#722ed1" strokeWidth="2">
								...
								<button onClick={logout} className="dropdown-item">
									<svg width="16" height="16" viewBox="0 0 19 19" fill="none">
										<path stroke="#1890FF" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M6.667 18H2.889A1.889 1.889 0 011 16.111V2.89A1.889 1.889 0 012.889 1h3.778M13.277 14.222L18 9.5l-4.723-4.722M18 9.5H6.667" />
									</svg>
									Log Out
								</button>
							</div>
